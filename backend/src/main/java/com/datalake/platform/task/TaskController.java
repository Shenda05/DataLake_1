...
public class TaskController {
    ...
    public ApiResponse<?> create(@Valid @RequestBody SaveTaskRequest body, HttpServletRequest request) {
        return ApiResponse.success(taskService.create(body, SecurityUtils.currentUser().userId()), requestId(request));
    }

    @PutMapping("/{taskId}")
    public ApiResponse<?> update(@PathVariable Long taskId, @Valid @RequestBody SaveTaskRequest body, HttpServletRequest request) {
        return ApiResponse.success(taskService.update(taskId, body), requestId(request));
    }
    ...
    public ApiResponse<?> resume(@PathVariable Long taskId, HttpServletRequest request) {
        return ApiResponse.success(taskService.resume(taskId, SecurityUtils.currentUser().userId()), requestId(request));
    }

...
public class DatasetService {
    ...
    public List<DatasetSummary> list() {
        return jdbcTemplate.query(
            """
                select dataset_id,dataset_name,source_id,format_type,record_count,field_count,status,creator,physical_table_name
                from data_set order by dataset_id desc
            """,
            (rs, rowNum) -> new DatasetSummary(
                ...
                rs.getString("status"),
                rs.getObject("creator") == null ? null : rs.getLong("creator"),
    ...
    public DatasetDetail detail(Long datasetId) {
        DatasetDetail detail = jdbcTemplate.query(
            """
                ...
                from data_set where dataset_id = ?
            """,
            rs -> rs.next()
                ? new DatasetDetail(
                    rs.getLong("dataset_id"),
    ...
    public void delete(Long datasetId) {
        Integer taskRef = jdbcTemplate.queryForObject("select count(*) from task_def where target_id = ?", Integer.class, datasetId);
        Integer flowRef = jdbcTemplate.queryForObject("select count(*) from governance_flow where input_dataset_id = ? or output_dataset_id = ?", Integer.class, datasetId, datasetId);
        if ((taskRef != null && taskRef > 0) || (flowRef != null && flowRef > 0)) {
            throw new IllegalArgumentException("该数据集已被流程或任务引用，不能删除");
        }
        DatasetDetail detail = detail(datasetId);

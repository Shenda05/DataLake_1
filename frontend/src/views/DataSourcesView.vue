...
async function submit() {
  try {
    ...
    Object.assign(form, { sourceName: '', sourceType: 'FILE', host: '', port: 3306, dbName: '', username: '', password: '', description: '' });
    await loadSources();
  } catch (error) {
    ElMessage.error(`数据源创建失败: ${(error as Error).message}`);
  }
}
  ...
  <div class="page-grid two-column-grid">
    <el-card shadow="never">
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="主机地址">
          <el-input v-model="form.host" :disabled="form.sourceType === 'FILE'" />
        </el-form-item>
        <el-form-item label="端口">
          <el-input-number v-model="form.port" :disabled="form.sourceType === 'FILE'" :min="1" :max="65535" />
        </el-form-item>
        <el-form-item label="数据库名">
          <el-input v-model="form.dbName" :disabled="form.sourceType === 'FILE'" />
        </el-form-item>
        <el-form-item label="用户名">
          <el-input v-model="form.username" :disabled="form.sourceType === 'FILE'" />
        </el-form-item>
        <el-form-item label="密码">
          <el-input v-model="form.password" type="password" :disabled="form.sourceType === 'FILE'" show-password />
        </el-form-item>
        <el-form-item label="说明">
          <el-input v-model="form.description" type="textarea" :rows="3" />
        </el-form-item>
        <el-button type="primary" @click="submit">保存数据源</el-button>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>数据源列表</span>
          <el-button link type="primary" @click="loadSources">刷新</el-button>
        </div>
      </template>

...
- 首页和日志页已经补齐跨模块快捷入口；首页新增电商核心指标（最近失败任务数、7 天订单/销售趋势、热销 Top5、低库存数量），统计卡片和核心图表支持点击跳转到对应模块页，日志页新增导入/治理/调度分类统计与失败原因聚合。
...
2. 电商仪表盘从数据库读取统计卡片、最近失败任务、近 7 天订单/销售趋势、热销商品 Top5 和低库存数量；点击统计卡片或核心图表可跳转到数据源、数据集、查询分析、任务或日志页面。
3. 管理员可以创建、编辑、测试、启停和删除数据源；`MYSQL` 数据源保存时支持同连接 `ALLOW / WARN / REJECT` 策略。
4. 电商数据接入页支持文件导入和 `MYSQL` 数据库表导入；文件导入支持业务域、编码方式、是否首行为表头等解析参数，数据库导入会自动识别 `Schema / Database` 下拉，切换后同步刷新表列表与预览，导入后自动生成数据集和元数据。

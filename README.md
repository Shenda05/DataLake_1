...
- 任务页和日志页现在支持筛选条件记忆、URL 查询参数回填；日志页还支持按当前筛选结果导出 `CSV / JSON`。
...
2. 可按状态、类型、关键字筛选日志。
3. 当前日志筛选会自动记忆，并同步到地址栏；需要留档时可直接导出当前筛选结果为 `CSV / JSON`。
4. 选中某条日志后，右侧会显示执行摘要、错误信息、耗时和操作用户，并可直接跳回任务页或来源模块。
5. 返回首页可展示最近任务、最近日志、任务趋势和“最近 1 分钟任务事件流”，适合答辩演示。
...
- 为任务页和日志页补充更多组合筛选项，例如执行目标、时间范围和最近失败记录。

...
- 任务页和日志页现在支持筛选条件记忆、URL 查询参数回填，并补齐了执行目标、时间范围、近 7 天失败记录等组合筛选；日志页还支持按当前筛选结果导出 `CSV / JSON`。
...
3. 当前任务页和日志页筛选都会自动记忆，并同步到地址栏。
4. 需要留档时可直接导出当前筛选结果为 `CSV / JSON`。
5. 选中某条日志后，右侧会显示执行摘要、错误信息、耗时和操作用户，并可直接跳回任务页或来源模块。
...
- 为任务页和日志页补充“保存常用筛选视图”和“失败日志一键回放”这类更适合演示的快捷操作。
